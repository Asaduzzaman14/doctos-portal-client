...
const AddDoctors = () => {
  ...
  const { data: services, isLoading } = useQuery("services", (req, res) =>
    fetch("https://docotrs-portal-server.vercel.app/service")
      .then((res) => res.json())
      .then()
  );
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className='form-control w-full max-w-xs'>
          ...
          <input
            ...
            type='file'
            className='input h-fit pl-0 input-bordered w-full max-w-xs'
          />
          <label className='label'>
            {errors.name?.type === "required" && (
              <span className='label-text-alt text-red-500'>
